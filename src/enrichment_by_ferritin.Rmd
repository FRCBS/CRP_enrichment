...
fr_filtered <- fr1997 %>%
    ...
    filter(IKA >= 18 & IKA <= 66) %>% # Filter away too young and too old
    #filter((HGB >= 125 & SUKUP == 2) | (HGB >= 135 & SUKUP == 1)) %>% # DON'T filter by hemoglobin, < 500 values in data
...
HbAbootmediandf <- data.frame(median = c(h00HbAmd$t0, h00HbAfilmd$t0),
                         lower = c(h00HbAmd$t0, h00HbAfilmd$t0),
                         upper = c(h00HbAmd$t0, h00HbAfilmd$t0),
                         filter = c("Unfiltered", "Filtered"),
                         cohort = c(rep("Health2000", 2)),
                         plotorder = c(0.9, 1.1))
```
...
HbA_median_p <- ggplot(data = HbAbootmediandf, aes(x = median, y = plotorder)) + 
    ...
    labs(title = "Population HbA1C (mmol/mol) median in Health2000, whether selected by ferritin >30 ug/l",
         subtitle = "95 % confidence interval (bootstrap) | No interval (all values the same)",
         x = "Median",
         color = "Ferritin > 30 ug/l")
```
...
if (!file.exists(paste0("~/CRP_enrichment/data/h2000fil_ferritin_threshold_boot", boot_n, ".rds"))){ # run bootstrap only if needed
    start <- Sys.time()
    # CRP
    CRP_diffs <- c()
    CRP_lower <- c()
    ...
    saveRDS(boot_diff_df, paste0("~/CRP_enrichment/data/h2000fil_ferritin_threshold_boot", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_diff_df <- readRDS(paste0("~/CRP_enrichment/data/h2000fil_ferritin_threshold_boot", boot_n, ".rds"))
}
```
...
if (!file.exists(paste0("~/CRP_enrichment/data/h2000fil_ferritin_x_SRhealth123_boot", boot_n, ".rds"))){ # run bootstrap only if needed
    start <- Sys.time()
    # Ferritin
    fer_diffs <- c()
    fer_lower <- c()
    ...
    # save to file so we don't have to do this every time
    saveRDS(boot_mean_df, paste0("~/CRP_enrichment/data/h2000fil_ferritin_x_SRhealth123_boot", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_mean_df <- readRDS(paste0("~/CRP_enrichment/data/h2000fil_ferritin_x_SRhealth123_boot", boot_n, ".rds"))
}
```
...
ferp <- ggplot(data = boot_mean_df[1:10, ], aes(x = LBs, y = diffs)) + 
    ...
    geom_line() +
    geom_vline(xintercept = 30, color = "red", alpha = 0.4) +
    geom_point(shape = 4) +
    theme_minimal() +
...
crpp <- ggplot(data = boot_mean_df[11:20, ], aes(x = LBs, y = diffs)) + 
    geom_ribbon(aes(ymin = lower, ymax = upper), fill = "#F87000", alpha = .2) +
    geom_line(color = "#F87000") +
    geom_vline(xintercept = 3, color = "red", alpha = 0.4) +
    geom_point(shape = 4) +
    theme_minimal() +
...
gpp <- ggplot(data = boot_mean_df[21:30, ], aes(x = LBs, y = diffs)) + 
    ...
    geom_vline(xintercept = 1.166, color = "red", alpha = 0.4) +
    geom_point(shape = 4) +
    theme_minimal() +
...
hbap <- ggplot(data = boot_mean_df[31:40, ], aes(x = LBs, y = diffs)) + 
    ...
    geom_point(shape = 4) +
    annotate("rect", xmin = 20, xmax = 42, ymin = 2, ymax = 3.25,
           alpha = .1,fill = "red") +
    theme_minimal() +
...
Let's stratify this between Females and Males next

## Gender strat

```{r genderstrat}
h2k_male <- h2000_filtered %>% filter(SP2 == 1)
h2k_female <- h2000_filtered %>% filter(SP2 == 2)
```

```{r marker_x_health_bootmean_genderstrat, echo = FALSE, warning = FALSE}
ferrLBs <- c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
CRPLBs <- c(0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5)
GPLBs <- as.numeric(quantile(h2000_filtered$GP, 0.1 * (0.9:9.9), na.rm = T))
HbA1CLBs <- c(18, 24, 30, 36, 42, 48, 54, 60, 66, 72)
if (!file.exists(paste0("~/CRP_enrichment/data/h2kgendered_ferritin_x_SRhealth123_boot", boot_n, ".rds"))) { # run bootstrap only if needed
    start <- Sys.time()
    # Ferritin MALE
    fer_diffs_m <- c()
    fer_lower_m <- c()
    fer_upper_m <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2k_male, statistic = get_mean_boot, R = boot_n, var1 = FERRITIINI, var2 = BA01, var1_trld = ferrLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        fer_diffs_m[i] <- boot_obj$t0
        fer_lower_m[i] <- ci_obj$normal[2]
        fer_upper_m[i] <- ci_obj$normal[3]
    }
    ...
    fer_upper_f <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2k_female, statistic = get_mean_boot, R = boot_n, var1 = FERRITIINI, var2 = BA01, var1_trld = ferrLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        fer_diffs_f[i] <- boot_obj$t0
        fer_lower_f[i] <- ci_obj$normal[2]
        fer_upper_f[i] <- ci_obj$normal[3]
    }
    ...
    CRP_upper_m <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2k_male, statistic = get_mean_boot, R = boot_n, var1 = CRP, var2 = BA01, var1_trld = CRPLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        CRP_diffs_m[i] <- boot_obj$t0
        CRP_lower_m[i] <- ci_obj$normal[2]
        CRP_upper_m[i] <- ci_obj$normal[3]
    }
    ...
    CRP_upper_f <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2k_female, statistic = get_mean_boot, R = boot_n, var1 = CRP, var2 = BA01, var1_trld = CRPLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        CRP_diffs_f[i] <- boot_obj$t0
        CRP_lower_f[i] <- ci_obj$normal[2]
        CRP_upper_f[i] <- ci_obj$normal[3]
    }
    ...
    GP_upper_m <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2k_male, statistic = get_mean_boot, R = boot_n, var1 = GP, var2 = BA01, var1_trld = GPLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        GP_diffs_m[i] <- boot_obj$t0
        GP_lower_m[i] <- ci_obj$normal[2]
        GP_upper_m[i] <- ci_obj$normal[3]
    }
    ...
    GP_upper_f <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2k_female, statistic = get_mean_boot, R = boot_n, var1 = GP, var2 = BA01, var1_trld = GPLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        GP_diffs_f[i] <- boot_obj$t0
        GP_lower_f[i] <- ci_obj$normal[2]
        GP_upper_f[i] <- ci_obj$normal[3]
    }
    ...
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2000_m, statistic = get_mean_boot, R = boot_n, var1 = hba, var2 = BA01, var1_trld = HbA1CLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        hba_diffs_m[i] <- boot_obj$t0
        hba_lower_m[i] <- ci_obj$normal[2]
        hba_upper_m[i] <- ci_obj$normal[3]
    }
    ...
    h2000_f <- h2k_female %>%
        mutate(hba = B_GHb_A1C * 10.93 - 23.50)
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2000_f, statistic = get_mean_boot, R = boot_n, var1 = hba, var2 = BA01, var1_trld = HbA1CLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        hba_diffs_f[i] <- boot_obj$t0
        hba_lower_f[i] <- ci_obj$normal[2]
        hba_upper_f[i] <- ci_obj$normal[3]
    }
    
    boot_mean_df <- data.frame(LBs = c(ferrLBs, ferrLBs, CRPLBs, CRPLBs, GPLBs, GPLBs, HbA1CLBs, HbA1CLBs),
                               diffs = c(fer_diffs_m, fer_diffs_f, CRP_diffs_m, CRP_diffs_f, GP_diffs_m, GP_diffs_f, hba_diffs_m, hba_diffs_f), 
                               lower = c(fer_lower_m, fer_lower_f, CRP_lower_m, CRP_lower_f, GP_lower_m, GP_lower_f, hba_lower_m, hba_lower_f), 
                               upper = c(fer_upper_m, fer_upper_f, CRP_upper_m, CRP_upper_f, GP_upper_m, GP_upper_f, hba_upper_m, hba_upper_f),
                               group = c(rep("Ferritin", 20), rep("CRP", 20), rep("GlycA", 20), rep("HbA1C", 20)),
                               gender = rep(c(rep("Male", 10), rep("Female", 10)), 4))
    
    # save to file so we don't have to do this every time
    saveRDS(boot_mean_df, paste0("~/CRP_enrichment/data/h2kgendered_ferritin_x_SRhealth123_boot", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_mean_df <- readRDS(paste0("~/CRP_enrichment/data/h2kgendered_ferritin_x_SRhealth123_boot", boot_n, ".rds"))
}
```

```{r SRH_plot_gendered, echo = FALSE, warning = FALSE, fig.height = 7, fig.width = 11, fig.align = 'center'}
ferp <- ggplot(data = boot_mean_df[1:20, ], aes(x = LBs, y = diffs)) + 
...
crpp <- ggplot(data = boot_mean_df[21:40, ], aes(x = LBs, y = diffs)) + 
    geom_ribbon(aes(ymin = lower, ymax = upper, group = gender, fill = gender), alpha = .2) +
    geom_line(aes(linetype = gender, color = gender)) +
    geom_vline(xintercept = 3, color = "red", alpha = 0.4) +
    geom_point(shape = 4) +
...
gpp <- ggplot(data = boot_mean_df[41:60, ], aes(x = LBs, y = diffs)) + 
    geom_ribbon(aes(ymin = lower, ymax = upper, group = gender, fill = gender), alpha = .2) +
    geom_line(aes(linetype = gender, color = gender)) +
    geom_vline(xintercept = 1.166, color = "red", alpha = 0.4) +
    geom_point(shape = 4) +
...
hbap <- ggplot(data = boot_mean_df[61:80, ], aes(x = LBs, y = diffs)) + 
    geom_ribbon(aes(ymin = lower, ymax = upper, group = gender, fill = gender), alpha = .2) +
    geom_line(aes(linetype = gender, color = gender)) +
    geom_point(shape = 4) +
    annotate("rect", xmin = 20, xmax = 42, ymin = 2, ymax = 4,
           alpha = .1,fill = "red") +
    theme_minimal() +
    labs(title = "by HbA1C level",
         x = "HbA1C (mmol/mol)",
...
## Pre- / postmenopause strat

```{r genderstrat}
premeno <- h2k_female %>% filter(MENOP == 3)
postmeno <- h2k_female %>% filter(MENOP == 1)
```

```{r marker_x_health_bootmean_meno_+_gender_strat, echo = FALSE, warning = FALSE}
ferrLBs <- c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
CRPLBs <- c(0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5)
GPLBs <- as.numeric(quantile(h2000_filtered$GP, 0.1 * (0.9:9.9), na.rm = T))
HbA1CLBs <- c(18, 24, 30, 36, 42, 48, 54, 60, 66, 72)
if (!file.exists(paste0("~/CRP_enrichment/data/h2kgendered_menop_ferritin_x_SRhealth_boot", boot_n, ".rds"))) { # run bootstrap only if needed
    start <- Sys.time()
    # Ferritin MALE
    fer_diffs_m <- c()
    fer_lower_m <- c()
    ...
    for (i in 1:length(ferrLBs)) {
        ...
        fer_diffs_m[i] <- boot_obj$t0
        fer_lower_m[i] <- ci_obj$normal[2]
        fer_upper_m[i] <- ci_obj$normal[t3]
    }
    
    # Ferritin PREMENOPAUSE FEMALE
    fer_diffs_pre <- c()
    fer_lower_pre <- c()
    fer_upper_pre <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(premeno, statistic = get_mean_boot, R = boot_n, var1 = FERRITIINI, var2 = BA01, var1_trld = ferrLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        fer_diffs_pre[i] <- boot_obj$t0
        fer_lower_pre[i] <- ci_obj$normal[2]
        fer_upper_pre[i] <- ci_obj$normal[3]
    }
    
    # Ferritin POSTMENOPAUSE FEMALE
    fer_diffs_post <- c()
    fer_lower_post <- c()
    fer_upper_post <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(postmeno, statistic = get_mean_boot, R = boot_n, var1 = FERRITIINI, var2 = BA01, var1_trld = ferrLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        fer_diffs_post[i] <- boot_obj$t0
        fer_lower_post[i] <- ci_obj$normal[2]
        fer_upper_post[i] <- ci_obj$normal[3]
    }
    ...
    CRP_upper_pre <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(premeno, statistic = get_mean_boot, R = boot_n, var1 = CRP, var2 = BA01, var1_trld = CRPLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        CRP_diffs_pre[i] <- boot_obj$t0
        CRP_lower_pre[i] <- ci_obj$normal[2]
        CRP_upper_pre[i] <- ci_obj$normal[3]
    }
    ...
    CRP_upper_post <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(postmeno, statistic = get_mean_boot, R = boot_n, var1 = CRP, var2 = BA01, var1_trld = CRPLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        CRP_diffs_post[i] <- boot_obj$t0
        CRP_lower_post[i] <- ci_obj$normal[2]
        CRP_upper_post[i] <- ci_obj$normal[3]
    }
    ...
    # GlycA PREMENO FEMALE
    GP_diffs_pre <- c()
    GP_lower_pre <- c()
    GP_upper_pre <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(premeno, statistic = get_mean_boot, R = boot_n, var1 = GP, var2 = BA01, var1_trld = GPLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        GP_diffs_pre[i] <- boot_obj$t0
        GP_lower_pre[i] <- ci_obj$normal[2]
        GP_upper_pre[i] <- ci_obj$normal[3]
    }
    
    # GlycA POSTMENO FEMALE
    GP_diffs_post <- c()
    GP_lower_post <- c()
    GP_upper_post <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(postmeno, statistic = get_mean_boot, R = boot_n, var1 = GP, var2 = BA01, var1_trld = GPLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        GP_diffs_post[i] <- boot_obj$t0
        GP_lower_post[i] <- ci_obj$normal[2]
        GP_upper_post[i] <- ci_obj$normal[3]
    }
    ...
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2000_f, statistic = get_mean_boot, R = boot_n, var1 = hba, var2 = BA01, var1_trld = HbA1CLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        hba_diffs_pre[i] <- boot_obj$t0
        hba_lower_pre[i] <- ci_obj$normal[2]
        hba_upper_pre[i] <- ci_obj$normal[3]
    }
    ...
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2000_f, statistic = get_mean_boot, R = boot_n, var1 = hba, var2 = BA01, var1_trld = HbA1CLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        hba_diffs_post[i] <- boot_obj$t0
        hba_lower_post[i] <- ci_obj$normal[2]
        hba_upper_post[i] <- ci_obj$normal[3]
    }
    
    boot_mean_df <- data.frame(LBs = c(rep(ferrLBs, 3), rep(CRPLBs, 3), rep(GPLBs, 3), rep(HbA1CLBs, 3)),
                               diffs = c(fer_diffs_m, fer_diffs_pre, fer_diffs_post, CRP_diffs_m, CRP_diffs_pre, CRP_diffs_post, GP_diffs_m, GP_diffs_pre, GP_diffs_post, hba_diffs_m, hba_diffs_pre, hba_diffs_post), 
                               lower = c(fer_lower_m, fer_lower_pre, fer_lower_post, CRP_lower_m, CRP_lower_pre, CRP_lower_post, GP_lower_m, GP_lower_pre, GP_lower_post, hba_lower_m, hba_lower_pre, hba_lower_post), 
                               upper = c(fer_upper_m, fer_upper_pre, fer_upper_post, CRP_upper_m, CRP_upper_pre, CRP_upper_post, GP_upper_m, GP_upper_pre, GP_upper_post, hba_upper_m, hba_upper_pre, hba_upper_post),
                               group = c(rep("Ferritin", 30), rep("CRP", 30), rep("GlycA", 30), rep("HbA1C", 30)),
                               strat = rep(c(rep("Male", 10), rep("Female PRE", 10), rep("Female POST", 10)), 4))
    
    # save to file so we don't have to do this every time
    saveRDS(boot_mean_df, paste0("~/CRP_enrichment/data/h2kgendered_menop_ferritin_x_SRhealth_boot", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_mean_df <- readRDS(paste0("~/CRP_enrichment/data/h2kgendered_menop_ferritin_x_SRhealth_boot", boot_n, ".rds"))
}
```

```{r SRH_plot_gendered_menop, echo = FALSE, warning = FALSE, fig.height = 9, fig.width = 16, fig.align = 'center'}
ferp <- ggplot(data = boot_mean_df[1:30, ], aes(x = LBs, y = diffs)) + 
    geom_ribbon(aes(ymin = lower, ymax = upper, group = strat, fill = strat), alpha = .2) +
    geom_line(aes(linetype = strat, color = strat)) +
    geom_vline(xintercept = 30, color = "red", alpha = 0.4) +
    geom_point(shape = 4) +
    theme_minimal() +
    labs(title = "Self reported health (Likert) mean by ferritin level",
...
crpp <- ggplot(data = boot_mean_df[31:60, ], aes(x = LBs, y = diffs)) + 
    geom_ribbon(aes(ymin = lower, ymax = upper, group = strat, fill = strat), alpha = .2) +
    geom_line(aes(linetype = strat, color = strat)) +
    geom_vline(xintercept = 3, color = "red", alpha = 0.4) +
    geom_point(shape = 4) +
...
gpp <- ggplot(data = boot_mean_df[61:90, ], aes(x = LBs, y = diffs)) + 
    geom_ribbon(aes(ymin = lower, ymax = upper, group = strat, fill = strat), alpha = .2) +
    geom_line(aes(linetype = strat, color = strat)) +
    geom_vline(xintercept = 1.166, color = "red", alpha = 0.4) +
    geom_point(shape = 4) +
...
hbap <- ggplot(data = boot_mean_df[91:120, ], aes(x = LBs, y = diffs)) + 
    geom_ribbon(aes(ymin = lower, ymax = upper, group = strat, fill = strat), alpha = .2) +
    geom_line(aes(linetype = strat, color = strat)) +
    geom_point(shape = 4) +
    annotate("rect", xmin = 20, xmax = 42, ymin = 0, ymax = 5,
           alpha = .1,fill = "red") +
    theme_minimal() +
    labs(title = "by HbA1C level",
         x = "HbA1C (mmol/mol)",
...
## Replicating Muriel's plot

```{r, fig.height = 8, fig.width = 4, fig.align = 'center'}

h2k_srh_fer_grouped <- h2000_filtered %>%
    transmute(SRH = case_when(BA01 == 1 ~ "Very good",
                              BA01 == 2 ~ "Good",
                              BA01 == 3 ~ "OK",
                              BA01 == 4 ~ "Bad",
                              BA01 == 5 ~ "Very bad"),
              Ferritin = FERRITIINI, 
              group = case_when(SP2 == 1 ~ "Men",
                                SP2 == 2 & MENOP == 3 ~ "Women PRE",
                                SP2 == 2 & MENOP == 1 ~ "Women POST")) %>%
    mutate(SRH = ordered(SRH, levels = c("Very bad", "Bad", "OK", "Good", "Very good")),
           group = ordered(group, levels = c("Women PRE", "Women POST", "Men"))) %>% drop_na()

ggplot(data = h2k_srh_fer_grouped, aes(x = SRH, y = Ferritin, color = group)) + 
    geom_hline(yintercept = 15, color = "dark red", size = .2, linetype = 2) +
    geom_boxplot(outlier.alpha = 0, alpha = 0, size = 0.3, width = 0.5) +
    geom_jitter(alpha = 0.3, shape = 1, size = 0.15, position = position_jitterdodge(jitter.height = 0, 
                                                                                     jitter.width = 0.5 )) +
...
### Now repeat ferritin x SRH with other cohorts

## FinRisk 1997

```{r, fig.height = 8, fig.width = 4, fig.align = 'center'}

fr_srh_fer_grouped <- fr_filtered %>%
    transmute(SRH = case_when(Q40 == 1 ~ "Very good",
                              Q40 == 2 ~ "Good",
                              Q40 == 3 ~ "OK",
                              Q40 == 4 ~ "Bad",
                              Q40 == 5 ~ "Very bad"),
              Ferritin = FERRITIN, 
              group = case_when(SUKUP == 1 ~ "Men",
                                SUKUP == 2 & K129 == 3 ~ "Women POST",
                                SUKUP == 2 & K129 != 3 ~ "Women PRE")) %>%
    mutate(SRH = ordered(SRH, levels = c("Very bad", "Bad", "OK", "Good", "Very good")),
           group = ordered(group, levels = c("Women PRE", "Women POST", "Men"))) %>% drop_na()

ggplot(data = fr_srh_fer_grouped, aes(x = SRH, y = Ferritin, color = group)) + 
    geom_hline(yintercept = 15, color = "dark red", size = .2, linetype = 2) +
    geom_boxplot(outlier.alpha = 0, alpha = 0, size = 0.3, width = 0.5) +
    geom_jitter(alpha = 0.3, shape = 1, size = 0.15, position = position_jitterdodge(jitter.height = 0, 
                                                                                     jitter.width = 0.5 )) +
...
```{r FinDonor_leftjoin, echo = FALSE}
findonor_all <- left_join(findonor, donors, by = "donor")
```

```{r, fig.height = 8, fig.width = 4, fig.align = 'center'}

fd_srh_fer_grouped <- findonor_all %>%
    transmute(SRH = case_when(QR17 == "Excellent" ~ "Very good",
                              QR17 == "Very_good" ~ "Good",
                              QR17 == "Good" ~ "OK",
                              QR17 == "Satisfactory" ~ "Bad",
                              QR17 == "Poor" ~ "Very bad",
                              QR17 == "NA" ~ NA_character_),
              Ferritin = Ferritin, 
              group = case_when(Gender == "Men" ~ "Men",
                                Gender == "Women" & (QR79 == "regular_period" | QR79 == "irregular_period") ~ "Women PRE",
                                Gender == 2 & QR79 == "no_period" ~ "Women POST",
...
Let's assume that people who report their health as "bad" or "really bad" (4, 5) don't come to donate. Does it affect our ratio analysis?
...
```{r, echo = FALSE}
ferrLBs <- c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
if (!file.exists(paste0("~/CRP_enrichment/data/h2000fil_sub4_ferritin_threshold_boot", boot_n, ".rds"))){ # run bootstrap only if needed
    start <- Sys.time()
    # CRP
    CRP_diffs <- c()
    CRP_lower <- c()
    ...
    # save to file so we don't have to do this every time
    saveRDS(boot_diff_df, paste0("~/CRP_enrichment/data/h2000fil_sub4_ferritin_threshold_boot", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_diff_df <- readRDS(paste0("~/CRP_enrichment/data/h2000fil_sub4_ferritin_threshold_boot", boot_n, ".rds"))
}
```
...
ggplot(data = boot_diff_df, aes(x = LB, y = diffs, color = group)) + 
    ...
    theme_minimal() +
    labs(title = "Growth of number of individuals over marker threshold per ferritin filter level",
         subtitle = paste("Cohort: Health2000 | Various donation restrictions applied | 95% CI from", boot_n, "bootstrap samples"),
         y = "Difference (pp)",
         x = "Ferritin filter (ug/l)",
         color = "Marker") + guides(fill = "none")
```

Further restriction by SRH < 4 doesn't seem to have any effect on the curves.
