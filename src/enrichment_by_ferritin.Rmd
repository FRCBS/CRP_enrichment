...
if (!file.exists(paste0("~/CRP_enrichment/data/h2000fil_ferritin_x_SRhealth_boot", boot_n, ".rds"))){ # run bootstrap only if needed
    start <- Sys.time()
    # Ferritin
    fer_diffs <- c()
    fer_lower <- c()
    ...
    # save to file so we don't have to do this every time
    saveRDS(boot_mean_df, paste0("~/CRP_enrichment/data/h2000fil_ferritin_x_SRhealth_boot", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_mean_df <- readRDS(paste0("~/CRP_enrichment/data/h2000fil_ferritin_x_SRhealth_boot", boot_n, ".rds"))
}
```
...
GPLBs <- as.numeric(quantile(h2000_filtered$GP, 0.1 * (0.9:9.9), na.rm = T))
HbA1CLBs <- c(18, 24, 30, 36, 42, 48, 54, 60, 66, 72)
if (!file.exists(paste0("~/CRP_enrichment/data/h2kgendered_ferritin_x_SRhealth_boot", boot_n, ".rds"))) { # run bootstrap only if needed
    start <- Sys.time()
    # Ferritin MALE
    fer_diffs_m <- c()
    fer_lower_m <- c()
    ...
    # save to file so we don't have to do this every time
    saveRDS(boot_mean_df, paste0("~/CRP_enrichment/data/h2kgendered_ferritin_x_SRhealth_boot", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_mean_df <- readRDS(paste0("~/CRP_enrichment/data/h2kgendered_ferritin_x_SRhealth_boot", boot_n, ".rds"))
}
```
...
How about FinRisk1997? It doesn't have HbA1C, but everything else should be OK. We also can't apply as many restrictions as with the H2000, because we're missing variables. 

```{r finrisk_filtered, echo = FALSE, include = FALSE}
fr_filtered <- fr1997 %>%
...
```{r marker_x_health_bootmean_fr, echo = FALSE, warning = FALSE, include = FALSE}
ferrLBs <- c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
CRPLBs <- c(0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5)
GPLBs <- as.numeric(quantile(fr_filtered$GP, 0.1 * (0.9:9.9), na.rm = T))
if (!file.exists(paste0("~/CRP_enrichment/data/fr97fil_ferritin_x_SRhealth_boot", boot_n, ".rds"))){ # run bootstrap only if needed
    start <- Sys.time()
    # Ferritin
    fer_diffs <- c()
    fer_lower <- c()
    fer_upper <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(fr_filtered, statistic = get_mean_boot, R = boot_n, var1 = FERRITIN, var2 = Q40, var1_trld = ferrLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        fer_diffs[i] <- boot_obj$t0
        fer_lower[i] <- ci_obj$normal[2]
        fer_upper[i] <- ci_obj$normal[3]
    ...
    CRP_upper <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(fr_filtered, statistic = get_mean_boot, R = boot_n, var1 = CRP, var2 = Q40, var1_trld = CRPLBs[i])
        ci_obj <- boot.ci(boot_obj, type = "norm")
        CRP_diffs[i] <- boot_obj$t0
        CRP_lower[i] <- ci_obj$normal[2]
        CRP_upper[i] <- ci_obj$normal[3]
    ...
    GP_lower <- c()
    GP_upper <- c()
    GP_median <- median(fr_filtered$GP, na.rm = T)
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(fr_filtered, statistic = get_mean_boot, R = boot_n, var1 = GP, var2 = Q40, var1_trld = GPLBs[i])
        GP_diffs[i] <- boot_obj$t0
        if (i == 10) { # quick hackfix for the fact that the last quantile has no bias or error (making CI fail)
            GP_lower[i] <- boot_obj$t0
            GP_upper[i] <- boot_obj$t0
            }
    }

    
    boot_mean_df <- data.frame(LBs = c(ferrLBs, CRPLBs, GPLBs),
                               diffs = c(fer_diffs, CRP_diffs, GP_diffs), 
                               lower = c(fer_lower, CRP_lower, GP_lower), 
                               upper = c(fer_upper, CRP_upper, GP_upper),
                               group = c(rep("Ferritin", 10), rep("CRP", 10), rep("GlycA", 10)))
    
    # save to file so we don't have to do this every time
    saveRDS(boot_mean_df, paste0("~/CRP_enrichment/data/fr97fil_ferritin_x_SRhealth_boot", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_mean_df <- readRDS(paste0("~/CRP_enrichment/data/fr97fil_ferritin_x_SRhealth_boot", boot_n, ".rds"))
}
```

```{r SRH_plot_fr97, echo = FALSE, warning = FALSE, fig.height = 7, fig.width = 11, fig.align = 'center', include = FALSE}
ferp <- ggplot(data = boot_mean_df[1:10, ], aes(x = LBs, y = diffs)) + 
    geom_ribbon(aes(ymin = lower, ymax = upper), alpha = .2) +
    geom_line() +
    geom_vline(xintercept = 30, color = "red", alpha = 0.4) +
    ...
    theme_minimal() +
    labs(title = "Self reported health (Likert) mean by ferritin level",
         subtitle = paste("Cohort: FINRISK1997 | Various donation restrictions applied | 95% CI from", boot_n, "bootstrap samples"),
         y = "Mean of self reported health",
         x = "Ferritin (ug/l)",
         color = "Marker") + guides(fill = "none")
...
multiplot(ferp, crpp, gpp, cols = 2)
```
