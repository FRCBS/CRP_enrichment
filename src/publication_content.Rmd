...
if (!file.exists(paste0("./data/PUBL_finrisk_CRP_", boot_n, ".rds"))) { # run bootstrap only if needed
    ...
    for (i in 1:iterations) {
        ...
        # Women|Menstr
        boot_obj_women_mens <- boot(fer_crp %>% filter(Group == "Women|Menstr" & Cohort == "FinRisk97"), statistic = get_ratio_boot, R = boot_n, 
                                   var1 = Ferritin, var2 = CRP, var1_trld = ferritin_values[i], var2_trld = CRP_trld, var2_over = T)
        ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
        # Women|Non-menstr
        boot_obj_women_nonmens <- boot(fer_crp %>% filter(Group == "Women|Non-menstr" & Cohort == "FinRisk97"), statistic = get_ratio_boot, R = boot_n, 
                                    var1 = Ferritin, var2 = CRP, var1_trld = ferritin_values[i],var2_trld = CRP_trld, var2_over = T)
        ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
    ...
    # Save
    bootob_list <- list(men = boot_obj_men, women_mens = boot_obj_women_mens, women_nonmens = boot_obj_women_nonmens)
    saveRDS(bootob_list, paste0("./data/PUBL_finrisk_CRP_", boot_n, ".rds"))
} else {
    bootob_list <- readRDS(paste0("./data/PUBL_finrisk_CRP_", boot_n, ".rds"))
    boot_obj_men <- bootob_list$men
    boot_obj_women_mens <- bootob_list$women_mens
    boot_obj_women_nonmens <- bootob_list$women_nonmens
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
    ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
    ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
...
if (!file.exists(paste0("./data/PUBL_health2k_CRP_", boot_n, ".rds"))) { # run bootstrap only if needed
    ...
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
    # Women|Menstr
    boot_obj_women_mens <- boot(fer_crp %>% filter(Group == "Women|Menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                               var1 = Ferritin, var2 = CRP, var1_trld = ferritin_values[i], var2_trld = CRP_trld, var2_over = T)
    ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
    # Women|Non-menstr
    boot_obj_women_nonmens <- boot(fer_crp %>% filter(Group == "Women|Non-menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                                var1 = Ferritin, var2 = CRP, var1_trld = ferritin_values[i], var2_trld = CRP_trld, var2_over = T)
    ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
    ...
    # Save
    bootob_list <- list(men = boot_obj_men, women_mens = boot_obj_women_mens, women_nonmens = boot_obj_women_nonmens)
    saveRDS(bootob_list, paste0("./data/PUBL_health2k_CRP_", boot_n, ".rds"))
} else {
    bootob_list <- readRDS(paste0("./data/PUBL_health2k_CRP_", boot_n, ".rds"))
    boot_obj_men <- bootob_list$men
    boot_obj_women_mens <- bootob_list$women_mens
    boot_obj_women_nonmens <- bootob_list$women_nonmens
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
...
if (!file.exists(paste0("./data/PUBL_finrisk_GlycA_", boot_n, ".rds"))) { # run bootstrap only if needed
    ...
    for (i in 1:iterations) {
        ...
        ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
        # Women|Menstr
        boot_obj_women_mens <- boot(fer_glyca %>% filter(Group == "Women|Menstr" & Cohort == "FinRisk97"), statistic = get_ratio_boot, R = boot_n, 
                                   var1 = Ferritin, var2 = GlycA, var1_trld = ferritin_values[i], var2_trld = GlycA_trld, var2_over = T)
        ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
        # Women|Non-menstr
        boot_obj_women_nonmens <- boot(fer_glyca %>% filter(Group == "Women|Non-menstr" & Cohort == "FinRisk97"), statistic = get_ratio_boot, R = boot_n, 
                                    var1 = Ferritin, var2 = GlycA, var1_trld = ferritin_values[i], var2_trld = GlycA_trld, var2_over = T)
        ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
    ...
    # Save
    bootob_list <- list(men = boot_obj_men, women_mens = boot_obj_women_mens, women_nonmens = boot_obj_women_nonmens)
    saveRDS(bootob_list, paste0("./data/PUBL_finrisk_GlycA_", boot_n, ".rds"))
} else {
    bootob_list <- readRDS(paste0("./data/PUBL_finrisk_GlycA_", boot_n, ".rds"))
    boot_obj_men <- bootob_list$men
    boot_obj_women_mens <- bootob_list$women_mens
    boot_obj_women_nonmens <- bootob_list$women_nonmens
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
...
if (!file.exists(paste0("./data/PUBL_health2k_GlycA_", boot_n, ".rds"))) { # run bootstrap only if needed
    ...
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
    # Women|Menstr
    boot_obj_women_mens <- boot(fer_glyca %>% filter(Group == "Women|Menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                               var1 = Ferritin, var2 = GlycA, var1_trld = ferritin_values[i], var2_trld = GlycA_trld, var2_over = T)
    ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
    # Women|Non-menstr
    boot_obj_women_nonmens <- boot(fer_glyca %>% filter(Group == "Women|Non-menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                                var1 = Ferritin, var2 = GlycA, var1_trld = ferritin_values[i], var2_trld = GlycA_trld, var2_over = T)
    ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
    ...
    # Save
    bootob_list <- list(men = boot_obj_men, women_mens = boot_obj_women_mens, women_nonmens = boot_obj_women_nonmens)
    saveRDS(bootob_list, paste0("./data/PUBL_health2k_GlycA_", boot_n, ".rds"))
} else {
    bootob_list <- readRDS(paste0("./data/PUBL_health2k_GlycA_", boot_n, ".rds"))
    boot_obj_men <- bootob_list$men
    boot_obj_women_mens <- bootob_list$women_mens
    boot_obj_women_nonmens <- bootob_list$women_nonmens
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
...
if (!file.exists(paste0("./data/PUBL_health2k_HbA1C_", boot_n, ".rds"))) { # run bootstrap only if needed
    ...
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
    # Women|Menstr
    boot_obj_women_mens <- boot(fer_hba1c %>% filter(Group == "Women|Menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                               var1 = Ferritin, var2 = HbA1C, var1_trld = ferritin_values[i], var2_trld = HbA1C_trld, var2_over = T)
    ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
    # Women|Non-menstr
    boot_obj_women_nonmens <- boot(fer_hba1c %>% filter(Group == "Women|Non-menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                                var1 = Ferritin, var2 = HbA1C, var1_trld = ferritin_values[i], var2_trld = HbA1C_trld, var2_over = T)
    ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
    ...
    # Save
    bootob_list <- list(men = boot_obj_men, women_mens = boot_obj_women_mens, women_nonmens = boot_obj_women_nonmens)
    saveRDS(bootob_list, paste0("./data/PUBL_health2k_HbA1C_", boot_n, ".rds"))
} else {
    bootob_list <- readRDS(paste0("./data/PUBL_health2k_HbA1C_", boot_n, ".rds"))
    boot_obj_men <- bootob_list$men
    boot_obj_women_mens <- bootob_list$women_mens
    boot_obj_women_nonmens <- bootob_list$women_nonmens
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
...
if (!file.exists(paste0("./data/PUBL_finrisk_APOB_", boot_n, ".rds"))) { # run bootstrap only if needed
    ...
    for (i in 1:iterations) {
        ...
        ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
        # Women|Menstr
        boot_obj_women_mens <- boot(fer_apob %>% filter(Group == "Women|Menstr" & Cohort == "FinRisk97"), statistic = get_ratio_boot, R = boot_n, 
                                   var1 = Ferritin, var2 = APOB, var1_trld = ferritin_values[i], var2_trld = APOB_trld_women, var2_over = T)
        ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
        # Women|Non-menstr
        boot_obj_women_nonmens <- boot(fer_apob %>% filter(Group == "Women|Non-menstr" & Cohort == "FinRisk97"), statistic = get_ratio_boot, R = boot_n, 
                                   var1 = Ferritin, var2 = APOB, var1_trld = ferritin_values[i], var2_trld = APOB_trld_women, var2_over = T)
        ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
    ...
    # Save
    bootob_list <- list(men = boot_obj_men, women_mens = boot_obj_women_mens, women_nonmens = boot_obj_women_nonmens)
    saveRDS(bootob_list, paste0("./data/PUBL_finrisk_APOB_", boot_n, ".rds"))
} else {
    bootob_list <- readRDS(paste0("./data/PUBL_finrisk_APOB_", boot_n, ".rds"))
    boot_obj_men <- bootob_list$men
    boot_obj_women_mens <- bootob_list$women_mens
    boot_obj_women_nonmens <- bootob_list$women_nonmens
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
...
if (!file.exists(paste0("./data/PUBL_health2k_APOB_", boot_n, ".rds"))) { # run bootstrap only if needed
    ...
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
    # Women|Menstr
    boot_obj_women_mens <- boot(fer_apob %>% filter(Group == "Women|Menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                               var1 = Ferritin, var2 = APOB, var1_trld = ferritin_values[i], var2_trld = APOB_trld_women, var2_over = T)
    ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
    # Women|Non-menstr
    boot_obj_women_nonmens <- boot(fer_apob %>% filter(Group == "Women|Non-menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                               var1 = Ferritin, var2 = APOB, var1_trld = ferritin_values[i], var2_trld = APOB_trld_women, var2_over = T)
    ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
    ...
    # Save
    bootob_list <- list(men = boot_obj_men, women_mens = boot_obj_women_mens, women_nonmens = boot_obj_women_nonmens)
    saveRDS(bootob_list, paste0("./data/PUBL_health2k_APOB_", boot_n, ".rds"))
} else {
    bootob_list <- readRDS(paste0("./data/PUBL_health2k_APOB_", boot_n, ".rds"))
    boot_obj_men <- bootob_list$men
    boot_obj_women_mens <- bootob_list$women_mens
    boot_obj_women_nonmens <- bootob_list$women_nonmens
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
...
if (!file.exists(paste0("./data/PUBL_finrisk_APOA1_", boot_n, ".rds"))) { # run bootstrap only if needed
    ...
    for (i in 1:iterations) {
        ...
        ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
        # Women|Menstr
        boot_obj_women_mens <- boot(fer_apoa1 %>% filter(Group == "Women|Menstr" & Cohort == "FinRisk97"), statistic = get_ratio_boot, R = boot_n, 
                                   var1 = Ferritin, var2 = APOA1, var1_trld = ferritin_values[i], var2_trld = APOA1_trld_women, var2_over = F)
        ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
        # Women|Non-menstr
        boot_obj_women_nonmens <- boot(fer_apoa1 %>% filter(Group == "Women|Non-menstr" & Cohort == "FinRisk97"), statistic = get_ratio_boot, R = boot_n, 
                                   var1 = Ferritin, var2 = APOA1, var1_trld = ferritin_values[i], var2_trld = APOA1_trld_women, var2_over = F)
        ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
    ...
    # Save
    bootob_list <- list(men = boot_obj_men, women_mens = boot_obj_women_mens, women_nonmens = boot_obj_women_nonmens)
    saveRDS(bootob_list, paste0("./data/PUBL_finrisk_APOA1_", boot_n, ".rds"))
} else {
    bootob_list <- readRDS(paste0("./data/PUBL_finrisk_APOA1_", boot_n, ".rds"))
    boot_obj_men <- bootob_list$men
    boot_obj_women_mens <- bootob_list$women_mens
    boot_obj_women_nonmens <- bootob_list$women_nonmens
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
...
if (!file.exists(paste0("./data/PUBL_health2k_APOA1_", boot_n, ".rds"))) { # run bootstrap only if needed
    ...
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
    # Women|Menstr
    boot_obj_women_mens <- boot(fer_apoa1 %>% filter(Group == "Women|Menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                               var1 = Ferritin, var2 = APOA1, var1_trld = ferritin_values[i], var2_trld = APOA1_trld_women, var2_over = F)
    ci_obj_women_mens <- boot.ci(boot_obj_women_mens, type = "norm")
    # Women|Non-menstr
    boot_obj_women_nonmens <- boot(fer_apoa1 %>% filter(Group == "Women|Non-menstr" & Cohort == "Health2k"), statistic = get_ratio_boot, R = boot_n, 
                               var1 = Ferritin, var2 = APOA1, var1_trld = ferritin_values[i], var2_trld = APOA1_trld_women, var2_over = F)
    ci_obj_women_nonmens <- boot.ci(boot_obj_women_nonmens, type = "norm")
    ...
    # Save
    bootob_list <- list(men = boot_obj_men, women_mens = boot_obj_women_mens, women_nonmens = boot_obj_women_nonmens)
    saveRDS(bootob_list, paste0("./data/PUBL_health2k_APOA1_", boot_n, ".rds"))
} else {
    bootob_list <- readRDS(paste0("./data/PUBL_health2k_APOA1_", boot_n, ".rds"))
    boot_obj_men <- bootob_list$men
    boot_obj_women_mens <- bootob_list$women_mens
    boot_obj_women_nonmens <- bootob_list$women_nonmens
    ci_obj_men <- boot.ci(boot_obj_men, type = "norm")
