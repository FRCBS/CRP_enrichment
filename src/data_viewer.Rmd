...
GP_median_p <- ggplot(data = bootmediandf, aes(x = median, y = plotorder)) + 
    geom_errorbarh(aes(xmin = lower, xmax = upper, color = filter), height = 0.06) +
    geom_point() +
    theme_minimal() +
    ylim(c(2, 0)) +
...
## Ferritin x HbA_1C (glycated hemoglobin)
...
Filter value for ferritin is set to a *lower bound*  of `r LB` ug/l for these analyses. Only the Health2000 cohort has this variable.
