...
frfilb <- boot(frfil$CRP, bootmean, R = 10000)
frb <- boot(fr$CRP, bootmean, R = 10000)
h00filb <- boot(h00fil$CRP, bootmean, R = 10000)
h00b <- boot(h00$CRP, bootmean, R = 10000)
donfilb <- boot(donfil$CRP, bootmean, R = 10000)
donb <- boot(don$CRP, bootmean, R = 10000)
...
frfilmd <- boot(frfil$CRP, bootmedian, R = 10000)
frmd <- boot(fr$CRP, bootmedian, R = 10000)
h00filmd <- boot(h00fil$CRP, bootmedian, R = 10000)
h00md <- boot(h00$CRP, bootmedian, R = 10000)
#donpop_omd <- boot(donpop_over$CRP, bootmedian, R = 10000)
#donpop_umd <- boot(donpop_under$CRP, bootmedian, R = 10000)
...
frGPfilb <- boot(frGPfil$GP, bootmean, R = 10000)
frGPb <- boot(frGP$GP, bootmean, R = 10000)
h00GPfilb <- boot(h00GPfil$GP, bootmean, R = 10000)
h00GPb <- boot(h00GP$GP, bootmean, R = 10000)
...
frGPfilmd <- boot(frGPfil$GP, bootmedian, R = 10000)
frGPmd <- boot(frGP$GP, bootmedian, R = 10000)
h00GPfilmd <- boot(h00GPfil$GP, bootmedian, R = 10000)
h00GPmd <- boot(h00GP$GP, bootmedian, R = 10000)
...
h00HbAfilb <- boot(h00HbAfil$HbAmm, bootmean, R = 10000)
h00HbAb <- boot(h00HbA$HbAmm, bootmean, R = 10000)
...
h00HbAfilmd <- boot(h00HbAfil$HbAmm, bootmedian, R = 10000)
h00HbAmd <- boot(h00HbA$HbAmm, bootmedian, R = 10000)
...
```{r prop_plot_NOT_RUN, eval=FALSE, fig.align='center', fig.height=5, fig.width=8, include=FALSE}
# TODO: THIS IS PROBABLY NOT NEEDED AS WE HAVE A WORKING BOOTSTRAPPED VERSION NOW
ferrLBs <- c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50)

# For CRP
CRP_diffs <- c()
for (i in 1:length(ferrLBs)){
    CRP_diffs[i] <- get_ratio(h2000, FERRITIINI, CRP, ferrLBs[i], 3)
}

# For GlycA
GP_diffs <- c()
for (i in 1:length(ferrLBs)){
    GP_diffs[i] <- get_ratio(h2000, FERRITIINI, GP, ferrLBs[i], median(h2000$GP, na.rm = T))
}
...
for (i in 1:length(ferrLBs)){
    hba_diffs[i] <- get_ratio(h2000_m, FERRITIINI, hba, ferrLBs[i], 42)
}

diffdf <- data.frame(LB = rep(ferrLBs, 3), diffs = c(CRP_diffs, GP_diffs, hba_diffs), group = c(rep("CRP", 10), rep("GP", 10), rep("HbA1C", 10)))

ggplot(data = diffdf, aes(x = LB, y = diffs, color = group)) + 
    geom_line() +
    geom_point(shape = 4) +
    theme_minimal() +
    labs(title = "Growth of number of individuals over marker threshold per ferritin filter level",
         subtitle = "Cohort: Health2000",
         y = "%",
         x = "Ferritin filter (ug/l)",
         color = "Marker")
```

```{r props_boot_BROKEN, eval=FALSE, include=FALSE}
# TODO: THIS IS BROKEN, USE BOOTHACK INSTEAD
boot_n <- 10000
if (!file.exists(paste0("~/CRP_enrichment/data/ferritin_threshold_boot", boot_n, ".rds"))){ # run bootstrap only if needed
    ...
    CRP_upper <- c()
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2000, statistic = get_ratio_boot, R = boot_n, var1 = FERRITIINI, var2 = CRP, var1_trld = ferrLBs[i], var2_trld = 3)
    ...
    GP_lower <- c()
    GP_upper <- c()
    GP_median <- median(h2000$GP, na.rm = T)
    for (i in 1:length(ferrLBs)) {
        boot_obj <- boot(h2000, statistic = get_ratio_boot, R = boot_n, var1 = FERRITIINI, var2 = GP, var1_trld = ferrLBs[i], var2_trld = GP_median)
        ci_obj <- boot.ci(boot_obj, type = "norm")
    ...
    boot_diff_df <- data.frame(LB = rep(ferrLBs, 3), diffs = c(CRP_diffs, GP_diffs, hba_diffs), 
                               lower = c(CRP_lower, GP_lower, hba_lower), upper = c(CRP_upper, GP_upper, hba_upper),
                                group = c(rep("CRP", 10), rep("GP", 10), rep("HbA1C", 10)))
...
```{r props_boothack, echo = FALSE}
ferrLBs <- c(5, 10, 15, 20, 25, 30, 35, 40, 45, 50)
boot_n <- 1000
if (!file.exists(paste0("~/CRP_enrichment/data/ferritin_threshold_boothack", boot_n, ".rds"))){ # run bootstrap only if needed
    start <- Sys.time()
    # CRP
    CRP_diffs <- c()
    CRP_lower <- c()
    ...
    for (i in 1:length(ferrLBs)) {
        est <- c()
        for (k in 1:boot_n) {
            est[k] <- get_ratio_boot(h2000, var1 = FERRITIINI, var2 = CRP, 
                                     var1_trld = ferrLBs[i], var2_trld = 3, i = sample(1:5251, 5251, replace = T))
        }
        
        qs <- quantile(est, probs = c(0.025, 0.975))
        CRP_diffs[i] <- mean(est)
        CRP_lower[i] <- qs[1]
        CRP_upper[i] <- qs[2]
    }
    # GlycA
    ...
    for (i in 1:length(ferrLBs)) {
        est <- c()
        for (k in 1:boot_n) {
            est[k] <- get_ratio_boot(h2000, var1 = FERRITIINI, var2 = GP, 
                                     var1_trld = ferrLBs[i], var2_trld = GP_median, i = sample(1:5251, 5251, replace = T))
        }
    ...
    for (i in 1:length(ferrLBs)) {
        est <- c()
        for (k in 1:boot_n) {
            est[k] <- get_ratio_boot(h2000_m, var1 = FERRITIINI, var2 = hba, 
                                     var1_trld = ferrLBs[i], var2_trld = 42, i = sample(1:5251, 5251, replace = T))
        }
        
        qs <- quantile(est, probs = c(0.025, 0.975))
        hba_diffs[i] <- mean(est)
        hba_lower[i] <- qs[1]
        hba_upper[i] <- qs[2]
    }
    ...
    # save to file so we don't have to do this every time
    saveRDS(boot_diff_df, paste0("~/CRP_enrichment/data/ferritin_threshold_boothack", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_diff_df <- readRDS(paste0("~/CRP_enrichment/data/ferritin_threshold_boothack", boot_n, ".rds"))
}
```


```{r prop_plot, echo = FALSE, fig.height=5, fig.width=8, fig.align = 'center'}
ggplot(data = boot_diff_df, aes(x = LB, y = diffs, color = group)) + 
    ...
    theme_minimal() +
    labs(title = "Growth of number of individuals over marker threshold per ferritin filter level",
         subtitle = paste("Cohort: Health2000 | 95% CI from", boot_n, "bootstrap samples"),
         y = "Difference (pp)",
         x = "Ferritin filter (ug/l)",
