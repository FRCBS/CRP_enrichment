...
boot_n <- 10000
if (!file.exists(paste0("~/CRP_enrichment/data/ferritin_threshold_boot_cut_", boot_n, ".rds"))){ # run bootstrap only if needed
    start <- Sys.time()
    # CRP
    CRP_diffs <- c()
    CRP_lower <- c()
    ...
    saveRDS(boot_diff_df, paste0("~/CRP_enrichment/data/ferritin_threshold_boot_cut_", boot_n, ".rds"))
    
    end <- Sys.time()
    print(end - start)
} else {
    boot_diff_df <- readRDS(paste0("~/CRP_enrichment/data/ferritin_threshold_boot_cut_", boot_n, ".rds"))
}
```
